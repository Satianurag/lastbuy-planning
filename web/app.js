...
function stages() {
  ...
  return `<section class="panel"><header class="panel-header"><div><h2>Specialist analysis</h2><p>${new Set(stages.map(s => s.result?.agent_session_id).filter(Boolean)).size > 1 ? "Recorded responses span multiple sessions. Current validation controls acceptance." : "Four bounded roles. One immutable snapshot."}${planBlocked(state.case) ? " Recorded model assessments below are subject to the blocking decision gates." : ""}</p></div><span class="badge">Foundry</span></header><div class="panel-body">${stages.map((s, i) => `<div class="stage ${esc(s.status)}"><span class="stage-number">${s.status === "COMPLETE" ? "✓" : i + 1}</span><div><h3>${labels[s.role]}</h3><p>${esc(descriptions[s.role])}</p>${s.result ? `<p>${esc(s.result.assessment.summary)}</p><button class="evidence-link execution-link" data-execution="${esc(s.role)}">Inspect recorded response ↗</button>` : ""}${s.error ? `<p>${esc(s.error)} · run did not complete</p>` : ""}</div><span class="stage-status">${esc(human(s.status))}</span></div>`).join("")}</div><div class="panel-footer">${stages.some((s) => s.result?.source === "live-foundry") ? "Recorded Foundry responses with evidence-tool execution. Complete means the stage finished; the case decision controls acceptance." : "No model results yet. Start an analysis to review live findings."}</div></section>`;
}
...
function executionDialog(role) {
  const stage = state.case.stages.find(s => s.role === role && s.result);
  if (!stage) return;
  const result = stage.result;
  const assessment = result.assessment || {};
  const mixed = new Set(state.case.stages.map(s => s.result?.agent_session_id).filter(Boolean)).size > 1;
  const blockers = state.case.plan?.blockers || [];
  const checks = assessment.checks || [];
  const quotes = checks.length ? "" : (assessment.findings || []).flatMap(f => f.supporting_quotes || []).map(q => `<blockquote>${esc(q.quote)}<cite>${esc(q.source_id)}</cite></blockquote>`).join("");
  const dialog = document.querySelector("#evidence-dialog");
  dialog.innerHTML = `<div class="dialog-heading"><div><div class="eyebrow">RECORDED PROVIDER RESPONSE</div><h2 id="evidence-title">${esc(labels[role])}</h2></div><button class="close" id="close-dialog" aria-label="Close execution proof">×</button></div><p class="note">Opening this record makes no model call. ${mixed ? "This case combines responses from different sessions; other roles may be reused reference results." : "This is the stored result of a completed specialist call."}</p><div class="source-meta"><div><span>AGENT / RELEASE</span>${esc(result.agent || "Not recorded")} · v${esc(result.agent_version || "?")}</div><div><span>MODEL DEPLOYMENT</span>${esc(result.model || "Not recorded")}</div><div><span>RECORDED TOOL</span>${esc((result.tool_calls || []).join(", ") || "None recorded")}</div><div><span>TRANSPORT</span>${esc(result.transport || result.source || "Not recorded")}</div></div><div class="proof-validation ${blockers.length ? "blocked" : ""}"><b>Current case validation: ${esc(human(state.case.status))}</b>${blockers.length ? `<p>${blockers.map(esc).join("<br>")}</p>` : ""}</div><details class="recorded-summary"><summary>Read recorded summary</summary><p>${esc(assessment.summary || "No summary recorded.")}</p></details><h3>Structured fields returned by the model</h3><p class="note">These are recorded observations, not approval. Each quote identifies the evidence used for that field.</p>${checks.map((check, i) => `<details class="field-check" ${i === 0 ? "open" : ""}><summary><span class="mono">${esc(check.key)}</span><b>${esc(check.observed_value ?? "Not established")}</b></summary>${(check.supporting_quotes || []).map(q => `<blockquote>${esc(q.quote)}<cite>Source: ${esc(q.source_id)}</cite></blockquote>`).join("")}</details>`).join("") || quotes || '<p class="note">No structured checks recorded.</p>'}<details class="execution-metadata"><summary>Response metadata</summary><p class="note mono">Session: ${esc(result.agent_session_id || "Not recorded")}<br>Prompt: ${esc(result.prompt_version || "Not recorded")}<br>Response-reported tokens: ${esc(result.usage?.total_token_count ?? "Not recorded")}<br>Model/tool duration: ${esc(result.duration_ms ?? "Not recorded")} ms (excludes end-to-end overhead)</p></details>`;
  dialog.showModal();
}
...
document.addEventListener("click", async (event) => {
  ...
  if (target.dataset.execution) {
    executionDialog(target.dataset.execution);
    return;
  }
