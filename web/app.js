...
function badge(status) {
  ...
  const bad = ["REJECTED", "ANALYSIS_FAILED", "INFEASIBLE", "FAILED"];
...
async function refresh() {
  ...
  const id = state.case?.id || state.cases[0]?.id;
...
function currentApprovals() {
  const c = state.case;
  if (
    ["STALE", "DRAFT", "ANALYZING", "ANALYSIS_FAILED", "REJECTED"].includes(
      c.status,
    )
  )
    return [];
...
function overview() {
  ...
  return `<div class="workspace"><div class="stack"><section class="panel"><header class="panel-header"><div><h2>From reported stock to a defensible purchase</h2><p>Each adjustment has an evidence trail.</p></div><span class="badge">EACH</span></header>${p ? `<div class="panel-body"><div class="bridge-row"><span class="label"><i class="swatch"></i>Reported on-hand inventory</span><span class="number">${n(p.reported_on_hand)}</span></div>${p.exclusions.map((x) => `<div class="bridge-row muted"><span class="label">− ${esc(x.reason)} <button class="evidence-link" data-source="${esc(x.evidence[0])}">Evidence ↗</button></span><span class="number">−${n(x.quantity)}</span></div>`).join("")}<div class="bridge-row"><span class="label"><i class="swatch green"></i>Eligible on-hand inventory</span><span class="number">${n(p.eligible_on_hand)}</span></div><div class="bridge-row"><span class="label"><i class="swatch gold"></i>Confirmed compatible inbound</span><span class="number">+${n(p.confirmed_inbound)}</span></div><div class="bridge-row total"><span>Required final purchase</span><span class="number">${n(p.purchase_quantity)} units</span></div>${p.purchase_quantity != null ? `<div class="equation"><span><b>${n(p.eligible_on_hand)}</b><small>ELIGIBLE STOCK</small></span>+<span><b>${n(p.confirmed_inbound)}</b><small>CONFIRMED INBOUND</small></span>+<span><b>${n(p.purchase_quantity)}</b><small>FINAL BUY</small></span>=<span><b>${n(p.eligible_on_hand + p.confirmed_inbound + p.purchase_quantity)}</b><small>PLANNED SUPPLY</small></span></div>` : ""}</div>` : '<div class="empty"><strong>The proposed purchase needs verification.</strong>Run the four specialists to reconcile coverage and stock.<br>The solver will calculate a constraint-checked purchase.</div>'}<div class="panel-footer">${p ? `Solver: ${esc(p.solver_status)} · ${p.allocations.length} cohort/date allocations independently verified` : "Signed coverage, released engineering and supplier constraints are required."}</div></section><section class="panel"><header class="panel-header"><div><h2>What changes the decision</h2><p>Governing evidence and specialist findings.</p></div></header><div class="panel-body">${
    findings.length
