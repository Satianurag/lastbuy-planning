...
| User's INR 1,000 total cap | Shared budget record: INR 700 reserved of INR 700 admission allowance; INR 300 infrastructure buffer | Another INR 40 full run is blocked with INR 0 unreserved. Actual charges unverified after billing HTTP 429. No new paid model runs for platform review or PDF creation. |
...
Hosted v7 completed a full synthetic cloud case. The current web release includes exact real public catalogue pricing with stock/freshness guards. The decision workflow and exporter include source-owner/currentness checks, cancellation and authoritative approval-wait expiry. Both old and new Durable orchestrators are registered. Actual local Functions/Azurite v2 checkpoint replay completed all four recorded stages and its human-approval wait; model responses were reused, not re-inferred. Cancellation/expiry branches also have offline tests. Local SQLite restore/reconciliation was tested; Azure SQL point-in-time restore was not.

Current release descriptors and curated evidence are indexed in `output/submission/package-manifest.json`. Earlier `final-release-check.json`, `browser-final-verification.json` and `cloud-export-verification.json` remain historical checkpoints; the current market release and dated cloud CREATE/RECOVER and historical-evidence gate records supersede them for those changed surfaces. Source archives use SHA-256 identifiers. Local Git retains the work; there is no public remote.

## Submission consistency audit

The canonical submission files are generated from `submission/entry.json`; software count comes from JUnit and price tables from `lastbuy/market_prices.json`. The PDF now includes all eight pages, real public pricing separately from example enterprise economics, the latest failure/refinement evidence and the exact same recording sequence. Current release hashes and artifact integrity are checked in `output/submission/package-manifest.json`.
